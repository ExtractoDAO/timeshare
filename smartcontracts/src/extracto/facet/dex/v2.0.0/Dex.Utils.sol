...
import {Auth} from "../../commodity/v2.0.0/Commodity.Auth.sol";
...
abstract contract Utils is Auth {
    ...
    ) internal view returns (bool result, uint256 index) {
        DexStorageLib.Storage storage lib = DexStorageLib.getDexStorage();

        DexStorageLib.Order[] memory orders = lib.orderBookMatch[amount][
            commodityAmount
        ];
    ...
    function filterOrdersByType(
        DexStorageLib.OrderType typed
    ) internal view returns (DexStorageLib.Order[] memory) {
        DexStorageLib.Storage storage lib = DexStorageLib.getDexStorage();
